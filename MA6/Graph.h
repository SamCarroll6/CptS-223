...
public:
    ...
    */
	...
	void dijkstra( int startingID ) {
		...
		for(auto vert : _vertices){
			dijkstra_queue.push(vert);
		}
		// End of hints - Dijkstra's Algorithm Goes here:
		Vertex * hold;
		//for(int i = 0; i < dijkstra_queue.size(); i++){
		while(!dijkstra_queue.empty()){
		int count = 0;
		hold = dijkstra_queue.top();
		unordered_map<Vertex *, double> edges = _vertices[hold->getId()]->getEdges();
		for(auto vert : _vertices){
			if(edges[vert] != 0 && edges[vert] != std::numeric_limits<double>::infinity()){
				if(hold->getPathWeight() + edges[vert] < vert->getPathWeight()){
					vert->setPathWeight(hold->getPathWeight() + edges[vert]);
					vert->set_path(hold);
				}
			}
		}
